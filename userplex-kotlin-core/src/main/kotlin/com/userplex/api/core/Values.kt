...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw UserplexInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw UserplexInvalidDataException("`$name` is not set")
            is JsonNull -> throw UserplexInvalidDataException("`$name` is null")
            else -> throw UserplexInvalidDataException("`$name` is invalid, received $this")
        }
